...
function Get-DTSEndpointPokerTableList {
    ...
    Write-DTCLog "Start endpoint ""pokergettablelist"" with data path ""$_poker_base_bath""" -Component "Get-DTSEndpointPokerTableList"
    ...
    Add-PodeRoute -Method Get -Path '/api/v1/dts/poker/gettablelist'-ScriptBlock {
        ...
        try {
            ...
            Write-PodeLog -Name "log" -InputObject @{Message="Initialize helper functions"; Component="Get-DTSEndpointPokerTableList"; Type="Info"}
            ...
            $_return_obj_list = Get-DTSEndpointHelperPokerGetTableList -PokerBasePath $PokerBasePath
...
function Get-DTSEndpointPokerCreateTable {
    ...
    Write-DTCLog "Start endpoint ""pokercreatetable"" with data path ""$_poker_base_bath""" -Component "Get-DTSEndpointPokerCreateTable"
    ...
    Add-PodeRoute -Method Get -Path '/api/v1/dts/poker/createtable'-ScriptBlock {
        ...
        try {
            ...
            Write-PodeLog -Name "log" -InputObject @{Message="Initialize helper functions"; Component="Get-DTSEndpointPokerCreateTable"; Type="Info"}
            . $PSScriptRoot\DTSEndpointsHelperPoker.ps1
            ...
            $_poker_table_password_hash = (Get-FileHash -InputStream $([IO.MemoryStream]::new([byte[]][char[]]"$($PokerTablePassword)$($_poker_table_password_salt)")) -Algorithm SHA256).Hash
            ...
            # Call function to read all poker table files
            Write-PodeLog -Name "log" -InputObject @{Message="Load poker tables from file system"; Component="Get-DTSEndpointPokerCreateTable"; Type="Info"}
            $_poker_table_list = Get-DTSEndpointHelperPokerGetTableList -PokerBasePath $PokerBasePath
            foreach($_poker_table in $_poker_table_list) {
                if($_poker_table.pokerTableName -eq $PokerTableName) {
                    throw "Poker table with this name already exist"
                }
            }
            ...
            $_poker_table_obj | Add-Member -MemberType NoteProperty -Name "pokerTablePassword" -Value $_poker_table_password_hash -Force
    ...
    Add-PodeRoute -Method Get -Path '/api/v1/dts/poker/gettable'-ScriptBlock {
        ...
        try {
            ...
            if($_poker_table_password_hash -ne $($_poker_table.pokerTablePassword)) {
                ...
                $_poker_table.PSObject.Properties.Remove("pokerTablePassword") 
