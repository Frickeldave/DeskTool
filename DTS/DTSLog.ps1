function Write-DTSLog {
    ...
    if($LogSource -eq "main") {
        Write-DTCLog -Message $Message -RefreshLogFile:$RefreshLogFile -Component $Component -Type $Type -LogFileDir "$PSScriptRoot\data" -LogFileName "DTS.log" -LogTarget "File"
    } else {
